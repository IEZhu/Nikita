...
### Fixed
- **Критическое:** служба падала целиком (`Exception while add_to_json '97'` → `graceful_shutdown(111)`), когда 1С дописывала новый код (пользователь/компьютер/событие/метаданные) в словарь `1Cv8.lgd` после старта службы: LGD-ветка парсера, в отличие от LGP, не перечитывала словарь. Теперь словарь перечитывается перед каждым блоком, а `add_to_json_data` при `KeyError` перечитывает словарь и повторяет запись один раз (для кода, отсутствующего и в файле, прежняя семантика остановки сохранена)
- **Критическое:** `graceful_shutdown()` из рабочего потока не завершал процесс службы — `sys.exit` убивает только вызвавший поток, главный поток `SvcDoRun` продолжал крутиться, и служба висела «зомби» (SCM показывает Running, recovery не срабатывает, все рабочие потоки остановлены). Заменено на `os._exit(exit_code)`
- Лог `Exception while add_to_json` теперь содержит маркер поля (`cc=N`), саму запись и полный traceback — раньше терялось и то, и другое
